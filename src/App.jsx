...
export default function App() {
  ...
  // Tight crop — face fills 70% of height
  let cropH=(fH*2.0)/0.70, cropW=cropH*asp;
  if(cropW>iW){ cropW=iW; cropH=cropW/asp; }
  if(cropH>iH){ cropH=iH; cropW=cropH*asp; }

  const headTop=fcY-fH*0.5-fH*0.5;
  const cY=Math.max(0,Math.min(headTop-cropH*0.05,iH-cropH));
  ...
  // High quality output
  return cv.toDataURL("image/jpeg", 1.0);
};
